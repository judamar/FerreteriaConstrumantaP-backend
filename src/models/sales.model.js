...
class Sales {
  ...
  static async getByUserName (userName) {
    ...
    return await pool.query('SELECT v.id AS venta_id, v.fecha_emision, u.nombre_completo AS nombre_cliente, ev.estado AS estado_venta, JSON_ARRAYAGG(JSON_OBJECT("cantidad", IFNULL(dv.cantidad_vendida, 0), "producto", IFNULL(p.nombre_producto, ""), "valor_unitario", IFNULL(p.precio, 0), "valor_sin_iva", IFNULL(p.precio / 1.19, 0), "valor_total", IFNULL(p.precio * dv.cantidad_vendida, 0))) AS productos, (SUM(IFNULL(p.precio * dv.cantidad_vendida, 0))/1.19) AS subototal, "19%" AS IVA, SUM(IFNULL(p.precio * dv.cantidad_vendida, 0)) AS total_venta FROM ventas AS v JOIN usuarios AS u ON v.usuarios_id = u.id JOIN estados_ventas AS ev ON v.estados_ventas_id = ev.id LEFT JOIN detalles_ventas AS dv ON v.id = dv.ventas_id LEFT JOIN productos AS p ON dv.productos_id = p.id WHERE v.nombre_completo LIKE ? GROUP BY v.id, v.fecha_emision, u.nombre_completo, ev.estado', [searchTerm])
      .then(([rows, fields]) => rows)
      .catch(err => {
        throw err
      })
