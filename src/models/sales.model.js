...
class Sales {
  static async create (sale) {
    return await pool.query('INSERT INTO ventas (usuarios_id, estados_ventas_id, tipo) VALUES (?, ?, ?)', [sale.usuarios_id, sale.estados_ventas_id, sale.tipo])
  ...
  static async update (id, sale) {
    return await pool.query('UPDATE ventas SET usuarios_id = ?, estados_ventas_id = ?, tipo = ? WHERE id = ?', [sale.usuarios_id, sale.estados_ventas_id, sale.tipo, id])
      .then(([rows, fields]) => rows)
      .catch(err => {
        throw err
      })
